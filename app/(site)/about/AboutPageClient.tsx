...
export default function AboutPageClient({ data }: AboutPageClientProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f8f6f3]">
          ...
          {data?.heroSections?.[0]?.backgroundImage?.asset?._ref ? (
            (() => {
              ...
              return (
                <Image
                  src={heroSrc}
                  alt="Love & Learning Child Care Center - About Us"
                  fill
                  priority
                  placeholder="blur"
                  blurDataURL={blurSrc}
          ...
          <div className="w-full flex flex-col lg:flex-row lg:items-center gap-16 lg:gap-24">
            ...
            <div className="relative w-full lg:w-[45%] h-[55vh] sm:h-[65vh] md:h-[75vh] lg:h-[70vh] min-h-[500px] max-h-[700px] order-2 lg:order-1 rounded-sm overflow-hidden shadow-sm">
              ...
              {data?.owners?.[0]?.image?.asset?._ref ? (
                <Image
                  src={urlFor(data.owners[0].image)
                    .width(800)
                    .quality(85)
                    .format("webp")
                    .url()}
                  alt={`${data.owners[0].name} - ${data.owners[0].role}`}
                  fill
                  placeholder="blur"
              ...
              <div className="w-full max-w-2xl mx-auto lg:mx-0">
                <div className="space-y-10 lg:space-y-12">
                  <div>
                    ...
                    <h2
                      id="owner-heading"
                      className="text-4xl sm:text-5xl lg:text-6xl font-light text-gray-900 mb-5 tracking-tight">
                      {data?.owners?.[0]?.name}
                    </h2>
                  ...
                  <div className="space-y-6 text-lg text-gray-600 leading-relaxed font-light prose prose-lg max-w-none prose-p:text-gray-600 prose-p:leading-relaxed prose-headings:text-gray-900 prose-headings:font-light">
                    {data?.owners?.[0]?.bio &&
                    Array.isArray(data.owners[0].bio) &&
                    data.owners[0].bio.length > 0 ? (
                      <PortableText
                        value={data.owners[0].bio}
      ...
      {/* Staff Section */}
      <section
        className="bg-[#ffd48b7e] py-16 md:py-24 lg:py-28 px-4 sm:px-6 lg:px-8"
        id="staff"
                ...
                <article
                  ...
                  <div className="relative overflow-hidden mb-6 md:mb-8 transition-transform duration-300 ease-out group-hover:scale-105">
                    {!staffImagesLoaded[member._key] && (
                      <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 w-full h-[25rem] sm:h-[28rem] md:h-[30rem] z-10">
                        <div
                          className="flex space-x-2"
                          role="status"
                          aria-label="Loading image">
                      ...
                      (expanded === member._key ? (
                        <>
                          ...
                          <button
                            ...
                            aria-label={`Show less about ${member.name}`}
                            aria-expanded="true"
                            className="text-gray-900 font-semibold hover:text-gray-700 transition-colors duration-300 underline decoration-2 underline-offset-4 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900">
                            Show less
                          </button>
                        </>
                      ) : (
